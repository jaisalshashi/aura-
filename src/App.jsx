...
function AuthScreen() {
  ...
  const handle = async () => {
    ...
    try {
      ...
      else if (e.code === "auth/invalid-credential") setError("Email ya password galat hai!");
  ...
  return (
    <div style={{ minHeight:"100vh", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", background:"linear-gradient(145deg,#050510,#0a0520)", padding:"20px" }}>
      ...
      <div style={{ width:"100%", maxWidth:380, background:"rgba(255,255,255,0.04)", backdropFilter:"blur(20px)", border:"1px solid rgba(255,255,255,0.09)", borderRadius:24, padding:"28px 24px" }}>
        ...
        {error && (
          <div style={{ fontFamily:"'Poppins',sans-serif", fontSize:12, color:"#f87171", marginBottom:14, padding:"10px 14px", borderRadius:10, background:"rgba(239,68,68,0.1)", border:"1px solid rgba(239,68,68,0.2)" }}>⚠️ {error}</div>
        )}
        ...
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start" }}>
          <div>
            ...
            <div style={{ fontSize:26, fontFamily:"'Playfair Display',serif", color:"#fff", marginTop:4, lineHeight:1.2 }}>
              {greeting}, <span style={{ color:"#a78bfa" }}>{userName}.</span>
            </div>
  ...
  useEffect(()=>{
    if(active){
      intervalRef.current=setInterval(()=>{
        setSeconds(s=>{
          if(s<=1){ setSessions(n=>n+1); setTotalMins(t=>t+25); setSessionLog(log=>[...log,{num:log.length+1,mins:25,time:new Date().toLocaleTimeString("en-US",{hour:"2-digit",minute:"2-digit"})}]); setActive(false); return 25*60; }
          return s-1;
        });
      },1000);
        ...
        <div style={{ display:"flex", alignItems:"flex-end", gap:8, height:100 }}>
          {weekData.map((v,i)=>(
            <div key={i} style={{ flex:1, display:"flex", flexDirection:"column", alignItems:"center", gap:6 }}>
              <div style={{ width:"100%", height:(v/maxBar)*80+"px", borderRadius:"6px 6px 0 0", background:i===6?"linear-gradient(180deg,#a78bfa,#7c3aed)":"rgba(167,139,250,0.22)" }}/>
...
function ProfileScreen({ xp, streak, habits, missions, userName, onLogout }) {
  ...
  return (
    <div style={{ padding:"32px 20px 100px" }}>
      ...
      <button onClick={onLogout} style={{ width:"100%", padding:"16px", borderRadius:14, border:"1px solid rgba(239,68,68,0.3)", background:"rgba(239,68,68,0.1)", color:"#f87171", fontFamily:"'Poppins',sans-serif", fontSize:14, cursor:"pointer", fontWeight:600 }}>
        Logout 👋
      </button>
    </div>
  );
        ...
        <div style={{ position:"fixed", bottom:0, left:"50%", transform:"translateX(-50%)", width:"100%", maxWidth:430, background:"rgba(5,5,20,0.88)", backdropFilter:"blur(30px)", WebkitBackdropFilter:"blur(30px)", borderTop:"1px solid rgba(255,255,255,0.07)", display:"flex", justifyContent:"space-around", alignItems:"center", padding:"10px 4px 18px", zIndex:100 }}>
          {tabs.map(t=>(
            <button key={t.id} onClick={()=>setScreen(t.id)} style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:4, border:"none", cursor:"pointer", padding:"6px 10px", borderRadius:14, transition:"all 0.2s", background:screen===t.id?"rgba(167,139,250,0.12)":"transparent" }}>
              <span style={{ fontSize:screen===t.id?22:18, color:screen===t.id?"#a78bfa":"rgba(255,255,255,0.3)", filter:screen===t.id?"drop-shadow(0 0 8px rgba(167,139,250,0.8))":"none" }}>{t.icon}</span>
