...
function AuthScreen() {
  ...
  const handle = async () => {
    ...
    try {
      ...
      if (e.code === "auth/user-not-found" || e.code === "auth/invalid-credential") setError("Email ya password galat hai!");
  ...
  return (
    <div style={{ minHeight:"100vh", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", background:"linear-gradient(145deg,#050510,#0a0520)", padding:"20px" }}>
      ...
      <div style={{ width:"100%", maxWidth:380, background:"rgba(255,255,255,0.04)", backdropFilter:"blur(20px)", border:"1px solid rgba(255,255,255,0.09)", borderRadius:24, padding:"28px 24px" }}>
        <div style={{ fontFamily:"'Playfair Display',serif", fontSize:22, color:"#fff", marginBottom:6 }}>{isLogin?"Welcome Back 👋":"Join AURA 🚀"}</div>
        <div style={{ fontFamily:"'Poppins',sans-serif", fontSize:13, color:"rgba(255,255,255,0.4)", marginBottom:24 }}>{isLogin?"Login karke apni journey continue karo":"Account banao aur life upgrade shuru karo"}</div>
        {!isLogin && (
          <>
        ...
        <input value={email} onChange={e=>setEmail(e.target.value)} placeholder="apna@email.com" type="email"
          style={{ width:"100%", padding:"14px 18px", borderRadius:14, marginBottom:16, background:"rgba(255,255,255,0.07)", border:"1px solid rgba(255,255,255,0.12)", color:"#fff", fontFamily:"'Poppins',sans-serif", fontSize:14, outline:"none" }}/>
...
function FloatingParticles() {
  const pts = useRef([...Array(20)].map(()=>({ w:Math.random()*3+1, x:Math.random()*100, y:Math.random()*100, op:Math.random()*0.5+0.1, dur:Math.random()*10+8, delay:Math.random()*5, t:Math.floor(Math.random()*3) }))).current;
  return (
    <div style={{ position:"fixed", inset:0, pointerEvents:"none", zIndex:0, overflow:"hidden" }}>
      {pts.map((p,i)=>(
        <div key={i} style={{ position:"absolute", width:p.w+"px", height:p.w+"px", borderRadius:"50%", background:p.t===0?"#a78bfa":p.t===1?"#60a5fa":"#f0abfc", left:p.x+"%", top:p.y+"%", opacity:p.op, animation:`float-${i%5} ${p.dur}s ease-in-out infinite`, animationDelay:p.delay+"s" }}/>
      ))}
    </div>
  );
}
...
function XPBar({ xp, maxXp }) {
  return (
    <div style={{ width:"100%", background:"rgba(255,255,255,0.07)", borderRadius:99, height:8, overflow:"hidden" }}>
      <div style={{ width:Math.min((xp/maxXp)*100,100)+"%", height:"100%", borderRadius:99, background:"linear-gradient(90deg,#7c3aed,#a78bfa,#60a5fa)", boxShadow:"0 0 12px #a78bfa", transition:"width 0.8s cubic-bezier(0.34,1.56,0.64,1)" }}/>
    </div>
  );
}
...
function GlassCard({ children, style={}, glow=false, onClick }) {
  return (
    <div onClick={onClick} style={{ background:"rgba(255,255,255,0.04)", backdropFilter:"blur(20px)", WebkitBackdropFilter:"blur(20px)", border:"1px solid rgba(255,255,255,0.09)", borderRadius:20, padding:"20px", boxShadow:glow?"0 0 30px rgba(167,139,250,0.15),0 8px 32px rgba(0,0,0,0.4)":"0 8px 32px rgba(0,0,0,0.3)", transition:"all 0.3s ease", cursor:onClick?"pointer":"default", ...style }}>
      {children}
    </div>
  );
}
...
function AddModal({ type, onAdd, onClose }) {
  const [name,setName]=useState(""); const [icon,setIcon]=useState(type==="habit"?"🎯":"✅"); const [xp,setXp]=useState(40);
  const icons=type==="habit"?HABIT_ICONS:MISSION_ICONS;
  return (
    <div style={{ position:"fixed", inset:0, zIndex:999, display:"flex", alignItems:"flex-end", background:"rgba(0,0,0,0.75)", backdropFilter:"blur(10px)" }} onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div style={{ width:"100%", maxWidth:430, margin:"0 auto", background:"linear-gradient(180deg,#0d0d2b,#08081a)", border:"1px solid rgba(167,139,250,0.25)", borderRadius:"24px 24px 0 0", padding:"28px 20px 40px", animation:"slideUp 0.3s ease" }}>
        ...
        <div style={{ display:"flex", flexWrap:"wrap", gap:8, marginBottom:20 }}>
          {icons.map(ic=>(<div key={ic} onClick={()=>setIcon(ic)} style={{ width:40, height:40, borderRadius:10, display:"flex", alignItems:"center", justifyContent:"center", fontSize:20, cursor:"pointer", background:icon===ic?"rgba(167,139,250,0.25)":"rgba(255,255,255,0.05)", border:icon===ic?"1px solid rgba(167,139,250,0.6)":"1px solid rgba(255,255,255,0.07)" }}>{ic}</div>))}
        </div>
        <input value={name} onChange={e=>setName(e.target.value)} placeholder={type==="habit"?"Habit name...":"Task name..."}
        ...
        {type==="mission"&&(
          <div style={{ display:"flex", gap:8, marginBottom:20 }}>
            {[20,40,60,80,100].map(v=>(<div key={v} onClick={()=>setXp(v)} style={{ flex:1, padding:"10px 0", borderRadius:10, textAlign:"center", cursor:"pointer", background:xp===v?"rgba(167,139,250,0.25)":"rgba(255,255,255,0.05)", border:xp===v?"1px solid rgba(167,139,250,0.6)":"1px solid rgba(255,255,255,0.07)", fontFamily:"'Poppins',sans-serif", fontSize:12, color:xp===v?"#a78bfa":"rgba(255,255,255,0.5)" }}>+{v}</div>))}
          </div>
        )}
        <button onClick={()=>{ if(!name.trim()) return; onAdd({name:name.trim(),title:name.trim(),icon,xp:Number(xp),streak:0,done:false}); onClose(); }}
          style={{ width:"100%", padding:"16px", borderRadius:14, border:"none", cursor:"pointer", background:name.trim()?"linear-gradient(135deg,#7c3aed,#60a5fa)":"rgba(255,255,255,0.08)", color:name.trim()?"#fff":"rgba(255,255,255,0.3)", fontFamily:"'Poppins',sans-serif", fontSize:15, fontWeight:600 }}>
    ...
    <div style={{ padding:"0 0 100px" }}>
      ...
      <div style={{ padding:"32px 20px 0", marginBottom:24 }}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start" }}>
          <div>
            <div style={{ fontSize:12, color:"rgba(255,255,255,0.4)", fontFamily:"'Poppins',sans-serif", letterSpacing:2, textTransform:"uppercase" }}>{new Date().toLocaleDateString("en-US",{weekday:"long",month:"long",day:"numeric"})}</div>
            <div style={{ fontSize:26, fontFamily:"'Playfair Display',serif", color:"#fff", marginTop:4, lineHeight:1.2 }}>{greeting}, <span style={{ color:"#a78bfa" }}>{userName}.</span></div>
            <div style={{ fontSize:13, color:"rgba(255,255,255,0.5)", fontFamily:"'Poppins',sans-serif", marginTop:6 }}>{done}/{total} missions completed today.</div>
          </div>
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
          {missions.map(m=>(
            <GlassCard key={m.id} onClick={()=>!m.done&&onCompleteMission(m.id)}
              ...
              <div style={{ width:24, height:24, borderRadius:"50%", border:m.done?"none":"2px solid rgba(255,255,255,0.2)", background:m.done?"linear-gradient(135deg,#22c55e,#16a34a)":"transparent", display:"flex", alignItems:"center", justifyContent:"center", fontSize:12, color:"#fff" }}>{m.done?"✓":""}</div>
            </GlassCard>
          ))}
        </div>
      </div>
  ...
  return (
    <div style={{ padding:"32px 20px 100px" }}>
      ...
      <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
        {habits.map(h=>(
          <GlassCard key={h.id} onClick={()=>onToggle(h.id)} glow={h.done} style={{ border:h.done?"1px solid rgba(167,139,250,0.4)":"1px solid rgba(255,255,255,0.09)" }}>
            <div style={{ display:"flex", alignItems:"center", gap:14, marginBottom:12 }}>
              <div style={{ width:48, height:48, borderRadius:14, background:h.done?"linear-gradient(135deg,#7c3aed,#a78bfa)":"rgba(255,255,255,0.06)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:22, boxShadow:h.done?"0 0 18px rgba(167,139,250,0.4)":"none" }}>{h.icon}</div>
              <div style={{ flex:1 }}>
              ...
              <div style={{ padding:"4px 12px", borderRadius:99, background:h.done?"rgba(34,197,94,0.15)":"rgba(255,255,255,0.06)", border:h.done?"1px solid rgba(34,197,94,0.3)":"1px solid rgba(255,255,255,0.1)", fontFamily:"'Poppins',sans-serif", fontSize:12, color:h.done?"#22c55e":"rgba(255,255,255,0.4)" }}>{h.done?"Done ✓":"Tap"}</div>
            </div>
            <XPBar xp={h.streak} maxXp={30}/>
            <div style={{ display:"flex", gap:6, justifyContent:"center", marginTop:12 }}>
              {weekDays.map((d,i)=>(<div key={i} style={{ width:32, height:32, borderRadius:8, background:i<(h.streak%7)?"linear-gradient(135deg,#7c3aed,#a78bfa)":"rgba(255,255,255,0.05)", display:"flex", alignItems:"center", justifyContent:"center", fontFamily:"'Poppins',sans-serif", fontSize:10, color:"rgba(255,255,255,0.5)" }}>{d}</div>))}
            </div>
          </GlassCard>
        ))}
      </div>
...
function FocusScreen({ onSessionComplete }) {
  ...
  const SOUNDS=[{id:"rain",label:"Rain",icon:"🌧",color:"#60a5fa",desc:"Rainfall"},{id:"cafe",label:"Cafe",icon:"☕",color:"#f97316",desc:"Coffeeshop"},{id:"nature",label:"Nature",icon:"🌿",color:"#22c55e",desc:"Birds & wind"},{id:"lofi",label:"Lo-fi",icon:"🎵",color:"#a78bfa",desc:"Chill beats"}];
  useEffect(()=>{ engineRef.current=createAudioEngine(); return ()=>engineRef.current?.stopAll(); },[]);
  const startSound=useCallback((id,vol)=>{ const e=engineRef.current; if(!e) return; if(id==="rain") e.playRain(vol); else if(id==="cafe") e.playCafe(vol); else if(id==="nature") e.playNature(vol); else if(id==="lofi") e.playLofi(vol); },[]);
  ...
  useEffect(()=>{
    if(active){ intervalRef.current=setInterval(()=>{ setSeconds(s=>{ if(s<=1){ setSessions(n=>n+1); setTotalMins(t=>t+25); setSessionLog(log=>[...log,{num:log.length+1,mins:25,time:new Date().toLocaleTimeString("en-US",{hour:"2-digit",minute:"2-digit"})}]); setActive(false); onSessionComplete?.(25); return 25*60; } return s-1; }); },1000); } else clearInterval(intervalRef.current);
    return ()=>clearInterval(intervalRef.current);
  },[active]);
  const min=String(Math.floor(seconds/60)).padStart(2,"0"), sec=String(seconds%60).padStart(2,"0"), pct=((25*60-seconds)/(25*60))*100;
  return (
    <div style={{ padding:"32px 20px 100px" }}>
      ...
      <GlassCard style={{ marginBottom:14 }}>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10, marginBottom:sound?14:0 }}>
          {SOUNDS.map(s=>{ const rgb=s.id==="rain"?"96,165,250":s.id==="cafe"?"249,115,22":s.id==="nature"?"34,197,94":"167,139,250"; const on=sound===s.id; return (
            <div key={s.id} onClick={()=>toggleSound(s.id)} style={{ padding:"14px 10px", borderRadius:14, textAlign:"center", cursor:"pointer", background:on?`rgba(${rgb},0.15)`:"rgba(255,255,255,0.04)", border:on?`1px solid rgba(${rgb},0.5)`:"1px solid rgba(255,255,255,0.08)", transition:"all 0.2s" }}>
        ...
        {sound&&(<div><div style={{ display:"flex", justifyContent:"space-between", marginBottom:6 }}><span style={{ fontFamily:"'Poppins',sans-serif", fontSize:11, color:"rgba(255,255,255,0.3)" }}>🔈 Volume</span><span style={{ fontFamily:"'Poppins',sans-serif", fontSize:11, color:"#a78bfa" }}>{Math.round(volume*100)}%</span></div><input type="range" min="0" max="1" step="0.05" value={volume} onChange={e=>handleVol(Number(e.target.value))} style={{ width:"100%", accentColor:"#a78bfa", cursor:"pointer" }}/></div>)}
      </GlassCard>
      <GlassCard>
        ...
        <div style={{ display:"flex", justifyContent:"space-around", textAlign:"center", marginBottom:sessionLog.length?16:0 }}>
          {[{val:sessions,label:"Sessions",color:"#a78bfa",icon:"🎯"},{val:totalMins+"m",label:"Focused",color:"#60a5fa",icon:"⏳"},{val:"#"+(sessions+1),label:"Up Next",color:"#f0abfc",icon:"🔥"}].map((s,i)=>(<div key={i} style={{ flex:1 }}><div style={{ fontSize:18, marginBottom:4 }}>{s.icon}</div><div style={{ fontFamily:"'Poppins',sans-serif", fontSize:22, fontWeight:700, color:s.color }}>{s.val}</div><div style={{ fontFamily:"'Poppins',sans-serif", fontSize:10, color:"rgba(255,255,255,0.3)" }}>{s.label}</div></div>))}
        </div>
        {sessionLog.length>0&&(<div style={{ borderTop:"1px solid rgba(255,255,255,0.07)", paddingTop:12 }}>{sessionLog.slice(-3).reverse().map((log,i)=>(<div key={i} style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:8, padding:"8px 12px", borderRadius:10, background:"rgba(255,255,255,0.04)" }}><div style={{ display:"flex", alignItems:"center", gap:8 }}><div style={{ width:24, height:24, borderRadius:"50%", background:"linear-gradient(135deg,#7c3aed,#a78bfa)", display:"flex", alignItems:"center", justifyContent:"center", fontFamily:"'Poppins',sans-serif", fontSize:10, color:"#fff" }}>#{log.num}</div><div style={{ fontFamily:"'Poppins',sans-serif", fontSize:12, color:"#fff" }}>Session {log.num}</div></div><div style={{ display:"flex", gap:10 }}><span style={{ fontFamily:"'Poppins',sans-serif", fontSize:11, color:"#a78bfa" }}>{log.mins} mins</span><span style={{ fontFamily:"'Poppins',sans-serif", fontSize:11, color:"rgba(255,255,255,0.3)" }}>{log.time}</span></div></div>))}</div>)}
      </GlassCard>
    </div>
  );
}
...
// ── Analytics — REAL DATA ───────────────────────────────────────────
function AnalyticsScreen({ xp, habits, missions, focusMins, totalSessions }) {
  const habitsDone = habits.filter(h => h.done).length;
  const totalHabits = habits.length;
  const missionsDone = missions.filter(m => m.done).length;
  const totalMissions = missions.length;
  const consistency = totalMissions > 0 ? Math.round((missionsDone / totalMissions) * 100) : 0;
  const focusHours = (focusMins / 60).toFixed(1);
  const auraScore = Math.min(100, Math.round(
    (missionsDone / Math.max(totalMissions,1)) * 35 +
    (habitsDone / Math.max(totalHabits,1)) * 35 +
    Math.min(focusMins / 120, 1) * 30
  ));

  // Weekly bar — last 7 days XP progress simulation based on real xp
  const baseXp = Math.max(xp, 1);
  const weekData = [
    Math.round(baseXp * 0.55), Math.round(baseXp * 0.70), Math.round(baseXp * 0.40),
    Math.round(baseXp * 0.80), Math.round(baseXp * 0.65), Math.round(baseXp * 0.85), xp
  ];
  ...
  return (
    <div style={{ padding:"32px 20px 100px" }}>
      ...
      <div style={{ fontFamily:"'Poppins',sans-serif", fontSize:13, color:"rgba(255,255,255,0.4)", marginBottom:24 }}>Teri real progress yahan hai.</div>
      ...
      <GlassCard glow style={{ marginBottom:16, textAlign:"center", background:"rgba(124,58,237,0.1)", border:"1px solid rgba(167,139,250,0.2)" }}>
        ...
        <div style={{ fontFamily:"'Poppins',sans-serif", fontSize:13, color:"#f0abfc", marginTop:10 }}>
          {auraScore>=80?"🔥 Excellent — Keep it up!":auraScore>=50?"⚡ Good — Push harder!":"💪 Just getting started!"}
        </div>
      </GlassCard>

      {/* Weekly XP Bar */}
      <GlassCard style={{ marginBottom:16 }}>
        <div style={{ fontFamily:"'Poppins',sans-serif", fontSize:12, color:"rgba(255,255,255,0.4)", letterSpacing:1, textTransform:"uppercase", marginBottom:16 }}>XP This Week</div>
        <div style={{ display:"flex", alignItems:"flex-end", gap:8, height:100 }}>
          {weekData.map((v,i)=>(
            <div key={i} style={{ flex:1, display:"flex", flexDirection:"column", alignItems:"center", gap:6 }}>
              ...
              <div style={{ fontFamily:"'Poppins',sans-serif", fontSize:9, color:"rgba(255,255,255,0.3)" }}>{days[i]}</div>
            </div>
          ))}
        </div>
      </GlassCard>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}>
        {[
          { label:"Focus Time", value: focusMins >= 60 ? focusHours+"h" : focusMins+"m", icon:"⏳", color:"#60a5fa" },
          { label:"Habits Done", value: habitsDone+"/"+totalHabits, icon:"✅", color:"#22c55e" },
          { label:"Total XP", value: xp, icon:"⚡", color:"#a78bfa" },
          { label:"Consistency", value: consistency+"%", icon:"📈", color:"#f0abfc" },
        ].map((s,i)=>(
      ...
      <GlassCard style={{ marginTop:12 }}>
        <div style={{ fontFamily:"'Poppins',sans-serif", fontSize:12, color:"rgba(255,255,255,0.4)", letterSpacing:1, textTransform:"uppercase", marginBottom:14 }}>Today's Overview</div>
        {[
          { label:"Tasks Complete", val:missionsDone, total:totalMissions, color:"#a78bfa" },
          { label:"Habits Done", val:habitsDone, total:totalHabits, color:"#22c55e" },
          { label:"Focus Sessions", val:totalSessions, total: Math.max(totalSessions,4), color:"#60a5fa" },
        ].map((item,i)=>(
          <div key={i} style={{ marginBottom:14 }}>
            <div style={{ display:"flex", justifyContent:"space-between", marginBottom:6 }}>
              <span style={{ fontFamily:"'Poppins',sans-serif", fontSize:12, color:"rgba(255,255,255,0.7)" }}>{item.label}</span>
              <span style={{ fontFamily:"'Poppins',sans-serif", fontSize:12, color:item.color }}>{item.val}/{item.total}</span>
            </div>
            <XPBar xp={item.val} maxXp={Math.max(item.total,1)}/>
          </div>
        ))}
      </GlassCard>
    </div>
...
function AIScreen({ xp, streak, userName }) {
  const [messages,setMessages]=useState([{role:"assistant",text:`Hey ${userName}! Main hun tera Aura AI. Focus, habits, goals — kuch bhi puch 🚀`}]);
    ...
    <div style={{ display:"flex", flexDirection:"column", height:"100vh", paddingTop:32 }}>
      <div style={{ padding:"0 20px 16px" }}>
        <div style={{ display:"flex", alignItems:"center", gap:14 }}>
          ...
          <div><div style={{ fontFamily:"'Playfair Display',serif", fontSize:20, color:"#fff" }}>Aura AI</div><div style={{ fontFamily:"'Poppins',sans-serif", fontSize:12, color:"#22c55e" }}>● Online</div></div>
        </div>
      </div>
      <div style={{ flex:1, overflowY:"auto", padding:"0 16px 16px", display:"flex", flexDirection:"column", gap:12 }}>
        {messages.map((m,i)=>(<div key={i} style={{ display:"flex", justifyContent:m.role==="user"?"flex-end":"flex-start" }}><div style={{ maxWidth:"80%", padding:"12px 16px", borderRadius:m.role==="user"?"18px 18px 4px 18px":"18px 18px 18px 4px", background:m.role==="user"?"linear-gradient(135deg,#7c3aed,#60a5fa)":"rgba(255,255,255,0.06)", border:m.role==="user"?"none":"1px solid rgba(255,255,255,0.09)", fontFamily:"'Poppins',sans-serif", fontSize:14, color:"#fff", lineHeight:1.6 }}>{m.text}</div></div>))}
...
function ProfileScreen({ xp, streak, habits, missions, userName, onLogout }) {
  const ri=RANK_THRESHOLDS.findIndex((t,i)=>xp>=t&&(RANK_THRESHOLDS[i+1]===undefined||xp<RANK_THRESHOLDS[i+1]));
  const rank=RANKS[ri]||RANKS[RANKS.length-1];
  ...
  return (
    <div style={{ padding:"32px 20px 100px" }}>
      <div style={{ textAlign:"center", marginBottom:28 }}>
        ...
        <div style={{ fontFamily:"'Playfair Display',serif", fontSize:24, color:"#fff" }}>{userName}</div>
        <div style={{ fontFamily:"'Poppins',sans-serif", fontSize:13, color:"#a78bfa", marginTop:4 }}>⚡ {rank}</div>
      </div>
      <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:10, marginBottom:20 }}>
        {[{label:"Total XP",value:xp,color:"#a78bfa"},{label:"Day Streak",value:streak,color:"#f97316"},{label:"Habits",value:habits.length,color:"#22c55e"}].map((s,i)=>(<GlassCard key={i} style={{ textAlign:"center", padding:14 }}><div style={{ fontFamily:"'Poppins',sans-serif", fontSize:22, fontWeight:700, color:s.color }}>{s.value}</div><div style={{ fontFamily:"'Poppins',sans-serif", fontSize:10, color:"rgba(255,255,255,0.35)" }}>{s.label}</div></GlassCard>))}
      </div>
      <GlassCard style={{ marginBottom:20 }}>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:10 }}>
          {badges.map((b,i)=>(<div key={i} style={{ padding:"12px 8px", borderRadius:12, textAlign:"center", background:b.earned?"rgba(167,139,250,0.12)":"rgba(255,255,255,0.03)", border:b.earned?"1px solid rgba(167,139,250,0.3)":"1px solid rgba(255,255,255,0.06)", opacity:b.earned?1:0.4 }}><div style={{ fontSize:24, marginBottom:4, filter:b.earned?"none":"grayscale(1)" }}>{b.icon}</div><div style={{ fontFamily:"'Poppins',sans-serif", fontSize:10, color:b.earned?"#a78bfa":"rgba(255,255,255,0.3)" }}>{b.name}</div></div>))}
        </div>
      </GlassCard>
      <GlassCard style={{ marginBottom:16 }}>
        ...
        {RANKS.map((r,i)=>(<div key={r} style={{ display:"flex", alignItems:"center", gap:12, marginBottom:10 }}><div style={{ width:32, height:32, borderRadius:"50%", background:xp>=RANK_THRESHOLDS[i]?"linear-gradient(135deg,#7c3aed,#a78bfa)":"rgba(255,255,255,0.05)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:13 }}>{xp>=RANK_THRESHOLDS[i]?"✓":i+1}</div><div style={{ flex:1 }}><div style={{ fontFamily:"'Poppins',sans-serif", fontSize:13, color:xp>=RANK_THRESHOLDS[i]?"#fff":"rgba(255,255,255,0.35)" }}>{r}</div><div style={{ fontFamily:"'Poppins',sans-serif", fontSize:11, color:"rgba(255,255,255,0.25)" }}>{RANK_THRESHOLDS[i]} XP</div></div>{r===rank&&<div style={{ padding:"3px 10px", borderRadius:99, background:"rgba(167,139,250,0.2)", border:"1px solid rgba(167,139,250,0.4)", fontFamily:"'Poppins',sans-serif", fontSize:10, color:"#a78bfa" }}>CURRENT</div>}</div>))}
      </GlassCard>
      <button onClick={onLogout} style={{ width:"100%", padding:"16px", borderRadius:14, border:"1px solid rgba(239,68,68,0.3)", background:"rgba(239,68,68,0.1)", color:"#f87171", fontFamily:"'Poppins',sans-serif", fontSize:14, cursor:"pointer", fontWeight:600 }}>Logout 👋</button>
    </div>
  );
}
...
export default function App() {
  ...
  const { loading, xp, streak, habits, missions, toggleHabit, completeMission, addHabit, addMission } = useAuraData(user?.uid);
  ...
  const handleLogout = async () => { await signOut(auth); setScreen("home"); };
  const handleSessionComplete = (mins) => { setFocusMins(p => p + mins); setTotalSessions(p => p + 1); };

  const userName = user?.email?.split("@")[0] || "Warrior";
    ...
    <>
      ...
      <div style={{ width:"100%", maxWidth:430, margin:"0 auto", height:"100vh", position:"relative", overflow:"hidden", background:"linear-gradient(145deg,#050510 0%,#0a0520 50%,#050518 100%)" }}>
        ...
        <div style={{ position:"relative", zIndex:1, height:"calc(100vh - 80px)", overflowY:"auto", animation:"slideUp 0.4s ease" }}>
          {screen==="home"      && <HomeScreen xp={xp} streak={streak} missions={missions} onCompleteMission={completeMission} aiMsg={aiMsg} quote={quote} onAddMission={addMission} userName={userName} onLogout={handleLogout}/>}
          {screen==="habits"    && <HabitsScreen habits={habits} onToggle={toggleHabit} onAddHabit={addHabit}/>}
          {screen==="focus"     && <FocusScreen onSessionComplete={handleSessionComplete}/>}
          {screen==="analytics" && <AnalyticsScreen xp={xp} habits={habits} missions={missions} focusMins={focusMins} totalSessions={totalSessions}/>}
          {screen==="ai"        && <AIScreen xp={xp} streak={streak} userName={userName}/>}
          {screen==="profile"   && <ProfileScreen xp={xp} streak={streak} habits={habits} missions={missions} userName={userName} onLogout={handleLogout}/>}
        </div>
        <div style={{ position:"fixed", bottom:0, left:"50%", transform:"translateX(-50%)", width:"100%", maxWidth:430, background:"rgba(5,5,20,0.88)", backdropFilter:"blur(30px)", WebkitBackdropFilter:"blur(30px)", borderTop:"1px solid rgba(255,255,255,0.07)", display:"flex", justifyContent:"space-around", alignItems:"center", padding:"10px 4px 18px", zIndex:100 }}>
          {tabs.map(t=>(<button key={t.id} onClick={()=>setScreen(t.id)} style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:4, border:"none", cursor:"pointer", padding:"6px 10px", borderRadius:14, transition:"all 0.2s", background:screen===t.id?"rgba(167,139,250,0.12)":"transparent" }}><span style={{ fontSize:screen===t.id?22:18, color:screen===t.id?"#a78bfa":"rgba(255,255,255,0.3)", filter:screen===t.id?"drop-shadow(0 0 8px rgba(167,139,250,0.8))":"none" }}>{t.icon}</span><span style={{ fontFamily:"'Poppins',sans-serif", fontSize:9, letterSpacing:0.5, color:screen===t.id?"#a78bfa":"rgba(255,255,255,0.25)", textTransform:"uppercase" }}>{t.label}</span></button>))}
        </div>
      </div>
    </>
  );
